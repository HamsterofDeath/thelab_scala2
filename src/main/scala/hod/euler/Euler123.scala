...
object Euler123 {
  ...
  def binary = {
    val src = allPrimesLong.zipWithIndex
                           .map { case (a, b) => (a, b + 1) }
                           .filter(_._2 % 2 == 1)
      .take(100000)
      .toVector
    ...
    def toCheckValue(index: Int) = {
      ...
      val check = (a + b) % modBy
      println(
        s"($p, $index) => (($p-1)^$index + ($p+1)^$index) % $modBy = $check"
      )
    ...
    sr match {
      ...
      case Searching.InsertionPoint(insertionPoint) =>
        println(src((insertionPoint)))
    }

  }

  def linear = {
    val firstPN = allPrimesLong.zipWithIndex
                               .map { case (a, b) => (a, b + 1) }
                               .find {
                                 case (p, index) =>
                                   index % 2 == 1 && {
                                     ...
                                     val check = (a + b) % modBy
                                     println(
                                       s"($p, $index) => (($p-1)^$index + ($p+1)^$index) % $modBy" +
                                       s" = $check"
                                     )
                                     check > limit
                                   }
