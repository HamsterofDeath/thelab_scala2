  ...
  def main(args: Array[String]): Unit = {
    ...
    def evalNext = {
      ...
      val nextNumber = {
        ...
        var sign = 1
        val element = relevantPositions.grouped(2).map { indexes =>
          val fiddlySum = indexes.map(where => partitionCounts(nextIndex-where) * sign).sum
          sign = -sign
          fiddlySum
        }.sum
    ...
    val solution = Iterator.continually(evalNext)
      .find(_ % 1000000 == 0)
