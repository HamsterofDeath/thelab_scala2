  ...
  def main(args: Array[String]): Unit = {
    ...
    case class Exponential(base: Int, power: Int) extends HasDoubleValue {
      ...
      def deExponentified(reduceBy: Int): HasDoubleValue = {
        if (reduceBy == power) Exponential(base, 1)
        else ExponentialD(base, power / reduceBy.toDouble)
      }
    ...
    val largest = in.max(
      Ordering.fromLessThan[Exponential]((a, b) => returnSmaller(a, b) == a)
    )
